...
for i in range(len(symbols)):
    ...
    data = stockLib.getRevenueOrEpsIncrease(db, symbol, startDate, endDate, increase=False)
    totalReturn = 0
    for d in data:
        # print(d)
        ...
        stockLib.savePatternStats(db, 7, d['date'], symbol, \
    ...
    print('Average 1m return for %s revenue or eps decrease between %s and %s: %6.02f%% (%d)\n' % (symbol, startDate, endDate, totalReturn*100, len(data)))
