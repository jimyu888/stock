...
for i in range(len(symbols)):
    ...
    data = stockLib.getRevenueIncrease(db, symbol, startDate, endDate, increase=False)
    totalReturn = 0
    for d in data:
        # print(d)
        ...
        stockLib.savePatternStats(db, 6, d['date'], symbol, \
    ...
    print('Average 1m return for %s revenue decrease between %s and %s: %6.02f%% (%d)\n' % (symbol, startDate, endDate, totalReturn*100, len(data)))
